...
#        self.yhat_dot = np.zeros(self.gtab.bvals.shape)
        self.exp_phi1 = np.zeros((self.small_delta.shape[0], 6))
        self.exp_phi1[:, 4] = np.exp(-self.yhat_ball)
        self.exp_phi1[:, 5] = np.ones(self.gtab.bvals.shape[0])
        self.x2 = np.zeros(6)
#        self.x1 = np.zeros(6)
...
#            x = res_one.x
        ...
        bounds = ([0.01, 0.01,  0.01, 0.01, 0.01, 0.1, 0.01,  0.01, 0.01, 0.01,
                   0.01, 0.1], [0.9,  0.9,  0.9, np.pi, np.pi, 16, 0.9,
...
#        x_fe[6] = fe[3]
    ...
    def Phi(self, x):
        ...
        self.exp_phi1[:, 1] = np.exp(-self.yhat_cylinder2)
        self.exp_phi1[:, 2] = np.exp(-self.yhat_zeppelin1)
...
#        x1 = x[0:3]
        S2_new(x[0:2], fe[0:2], self.gtab.bvals,  self.gtab.bvecs, self.D_intra,
               self.yhat_zeppelin1)
        S2_new(x[3:5], fe[3:5], self.gtab.bvals,  self.gtab.bvecs, self.D_intra,
               self.yhat_zeppelin2)
