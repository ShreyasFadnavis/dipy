# -*- coding: utf-8 -*-
"""
Created on Tue Dec  5 16:23:14 2017
...
#        1e-8 default, 1e-3
        ...
        self.td = self.big_delta - self.small_delta/3
        self.gamma = gamma
        ...
        self.exp_phi1 = np.zeros((self.small_delta.shape[0], 2))
        self.x2 = np.zeros(3)
        self.T1 = 0.832
        self.TM = self.big_delta - self.te/2
    ...
    def fit(self, data):
        ...
        """
        bounds = np.array([(0.11, 16), (0.11, 0.8), (0.01, 0.8), (0.01, 80)])
...
#    def S3(self):
#        # ball
#        yhat_ball = self.D_iso * self.gtab.bvals
#        return yhat_ball
#
#    def S4(self):
#        # dot
...
#        return yhat_dot
    ...
    def x_and_fe_to_x_fe(self, x, fe):
        ...
        fe = fe[:, 0]
        x_fe[:1] = fe[:1]
        x_fe[1] = self.theta_angle[0]
        x_fe[2] = self.phi_angle[0]
        x_fe[3] = x[0]
        x_fe[4] = x[2]
        return x_fe
    ...
    def Phi(self, x):
        ...
        self.exp_phi1[:, 0] = np.exp(-self.TM/self.T1)*np.exp(-self.yhat_cylinder - self.td/tau1) \
            + np.exp(-self.TM/self.T1)*np.exp(self.td/tau1)
        self.exp_phi1[:, 1] = np.exp(-self.TM/self.T1)*np.exp(-self.yhat_zeppelin - self.td/tau2) \
            + np.exp(-self.TM/self.T1)*np.exp(self.td/tau2)
    ...
    def Phi2(self, x_fe):
        ...
        tau = x[3]
        fe1 = np.array([fe, 1 - fe])
        S2_new(x, fe1, self.gtab.bvals,  self.gtab.bvecs, self.D_intra,
               self.yhat_zeppelin)
        S1(x1, am, self.gtab.bvecs, self.gtab.bvals, self.gtab.small_delta,
           self.gtab.big_delta, self.G2, self.L, self.D_intra,
           self.yhat_cylinder)
        self.exp_phi1[:, 0] = np.exp(-self.TM/self.T1)*np.exp(-self.yhat_cylinder - self.td/tau) \
            + np.exp(-self.TM/self.T1)*np.exp(self.td/tau)
        self.exp_phi1[:, 1] = np.exp(-self.TM/self.T1)*np.exp(-self.yhat_zeppelin - self.td/tau*fe1[1]/fe1[0]) \
            + np.exp(-self.TM/self.T1)*np.exp(self.td/tau*fe1[1]/fe1[0])
        return self.exp_phi1
