# -*- coding: utf-8 -*-
"""
Created on Tue Dec 05 11:33:04 2017
@author: Maryam
"""
...
#        x_fe[6] = fe[3]
    ...
    def Phi(self, x):
        ...
        self.exp_phi1[:, 0] = np.exp(-self.yhat_cylinder - self.td/tau1) \
            + np.exp(self.td/tau1)
        self.exp_phi1[:, 1] = np.exp(-self.yhat_zeppelin - self.td/tau2) \
            + np.exp(self.td/tau2)
    ...
    def Phi2(self, x_fe):
        ...
        S1(x1, am, self.gtab.bvecs, self.gtab.bvals, self.gtab.small_delta,
           ...
           self.yhat_cylinder)
        self.exp_phi1[:, 0] = np.exp(-self.yhat_cylinder - self.td/tau) \
            + np.exp(self.td/tau)
        self.exp_phi1[:, 1] = np.exp(-self.yhat_zeppelin - self.td/tau*fe1[1]/fe1[0]) \
            + np.exp(self.td/tau*fe1[1]/fe1[0])
        return self.exp_phi1
