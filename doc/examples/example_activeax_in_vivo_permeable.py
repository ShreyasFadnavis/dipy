# -*- coding: utf-8 -*-
"""
Created on Tue Dec 05 11:53:17 2017
@author: Maryam
"""
...
import dipy.reconst.activeax_in_vivo_permeable as activeax_in_vivo_permeable
...
fit_method = 'MIX'
activeax_model = activeax_in_vivo_permeable.ActiveAxModel(gtab, params, D_intra, D_iso, fit_method=fit_method)
#activeax_model = activeax.ActiveAxModel(gtab, fit_method=fit_method)
activeax_fit = np.zeros((Y.shape[0], Y.shape[1], 1, 5))
mask[0:127,:] = mask[128:0:-1,:]

t1 = time()
for i in range(80, 81):  #range(Y.shape[0]):
    for j in range(30, 31):  #range(Y.shape[1]):
        for k in range(1):
            if mask[i, j, k] > 0:
