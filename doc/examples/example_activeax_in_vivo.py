# -*- coding: utf-8 -*-
"""
Created on Tue Nov 07 16:44:05 2017
...
import dipy.reconst.activeax_in_vivo as activeax_in_vivo
...
fit_method = 'MIX'
activeax_model = activeax_in_vivo.ActiveAxModel(gtab, params, D_intra, D_iso, fit_method=fit_method)
#activeax_model = activeax.ActiveAxModel(gtab, fit_method=fit_method)
activeax_fit = np.zeros((Y.shape[0], Y.shape[1], 1, 7))
...
nib.save(nib.Nifti1Image(activeax_fit[:, :, :, 0], affine), 'f1_in_vivo_activeax_CC.nii.gz')
nib.save(nib.Nifti1Image(activeax_fit[:, :, :, 1], affine), 'f2_in_vivo_activeax_CC.nii.gz')
nib.save(nib.Nifti1Image(activeax_fit[:, :, :, 2], affine), 'f3_in_vivo_activeax_CC.nii.gz')
nib.save(nib.Nifti1Image(activeax_fit[:, :, :, 3], affine), 'theta_in_vivo_activeax_CC.nii.gz')
nib.save(nib.Nifti1Image(activeax_fit[:, :, :, 4], affine), 'phi_in_vivo_activeax_CC.nii.gz')
